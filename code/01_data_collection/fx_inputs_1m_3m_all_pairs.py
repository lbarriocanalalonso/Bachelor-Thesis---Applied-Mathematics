...
out_xlsx = "fx_inputs_1m_3m_ALL_PAIRS_LIBOR_ONLY_LATEST.xlsx"
...
# Each entry is a list of IBOR-style tickers 
#
...
# Optional hard overrides if Bloomberg has a special ticker for one currency/tenor
...
def build_rate_candidates(ccy: str, tenor: str) -> List[str]:
    ...
    That means: no fallback variants. 
    """
    ccy = ccy.upper()
    tenor = tenor.upper()
...
for pair in all_pairs:
    ...
    # Keep only rows where we can compute BOTH 1M and 3M forwards (spot + both currencies' rates)
    req = [
        "spot",
        f"r_f_{base}_1M",
        f"r_d_{quote}_1M",
    ...
    valid = sheet[req].notna().all(axis=1)
    if not valid.any():
        print(f"[{pair}] No dates with complete spot+rates for both tenors (skip pair)")
        continue

    last_dt = sheet.index[valid].max()
    sheet = sheet.loc[:last_dt].dropna(subset=req)

    # Final format
    sheet = sheet.reset_index().rename(columns={"index": "date"})

    # Record per-pair coverage for auditing
    pair_cov_rows.append(
        {
            "pair": pair,
            "first_date": sheet["date"].min(),
            "last_date": sheet["date"].max(),
...
pair_cov_df = pd.DataFrame(pair_cov_rows).sort_values(["pair"])
...
with pd.ExcelWriter(out_xlsx, engine="openpyxl") as xw:
    ...
    pair_cov_df.to_excel(xw, sheet_name="PAIR_COVERAGE", index=False)
